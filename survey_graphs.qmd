...
Amherst, MA  
https://jeffreyblanchard.github.io/lab/
...
These graphs, tables and open responses are from an anonymous Google forms survey that was designed to solicit students’ personal experience on the effectiveness of using generative AI in this course. The input files with the with the raw survey results and the thematic coding of the open responses can be found in in the course [GitHub repository](https://github.com/jeffreyblanchard/PathoGen2025/tree/main/survey). These results are generated from this csv file and the underlying R code can also be found in the above GitHub repo and are rendered on this [survey page](https://jeffreyblanchard.github.io/PathoGen2025/survey.html). The R code for producing many of these graphs was developed interactively using the UMass Amherst GenAI platform with OpenAI GPT-5-mini or Anthropic Claude 4.6 Opus. 

The student's perspectives from this survey are the basis of ["Learning R with generative AI in a metagenomic data science course"}(https://doi.org/10.1128/jmbe.00343-25 ) published in a Special Series of the Journal of Microbiology and Biology Education on the Practical and Positive Uses of AI in STEM.

### Figure S1. Prior Coding Experience 
...
Prior_Coding_Experience <- df_survey |>
...
#ggsave("survey/Prior_Coding_Experience.jpg", width = 10, height = 2)
```

### Figure S2. Frequency of AI Use
...
Frequency_of_AI_Use
#ggsave("survey/Frequency_of_AI_Use.jpg", width = 10, height = 2)
```

### Figure S3. How much time did using AI save me?
...
How_much_time
#ggsave("survey/How_much_time.jpg", width = 10, height = 2)
```

### Figure S4. Frequency of incorrect AI output
...
Frequency_of_incorrect_AI <- df_survey |>
...
Frequency_of_incorrect_AI
#ggsave("survey/Frequency_of_incorrect_AI.jpg", width = 10, height = 2)
```

### Figure S5. Did you become more engaged or interactive in solving problems using AI?
...
Did_you_become_more_engaged <- df_survey |>
...
Did_you_become_more_engaged
#ggsave("survey/Did_you_become_more_engaged.jpg", width = 10, height = 2)
```

###  Figure S6. Confidence change in programming in R 
...
Confidence_change
#ggsave("survey/Confidence_change.jpg", width = 10, height = 2)
```

###  Figure S7. Clarity of instructions on AI use
...
Clarity_of_instructions <- df_survey |>
...
#ggsave("survey/Clarity_of_instructions.jpg", width = 10, height = 2)
```

### Figure S8. Combined likert graph of multiple choice grids
...
#ggsave("survey/combined_likert.jpg", width = 11, height = 15)
```

###  Figure S9. When AI was wrong, I typically…
...
p
# ggsave("survey/when_AI_wrong.jpg", width = 10, height = 5)
```

###  Figure S10. Common issues encountered
...
p
# ggsave("survey/common_issues.jpg", width = 10, height = 4)
```

###  Figure S11. Where more support would help
...
p
# ggsave("survey/more_support.jpg", width = 10, height = 4)
```

### Table S1 - Overall effectiveness / Likelihood to recommend AI
...
res <- df |>
  select(any_of(cols)) |>
  pivot_longer(everything(), names_to = "Prompt", values_to = "value") |>
...
### Table S2. Examples of helpful AI responses - Open response
...
### Table S3. Examples of unhelpful or incorrect AI responses - Open response
...
### Table S4. Suggested changes for future offering - Open response
...
### Table S5. Final comments and suggestions - Open response
...
### Analysis of open responses using common themes - Overview

All open-text responses from four survey sections (Above Tables S2-S5) were systematically reviewed and coded thematically. A coding scheme was developed inductively from the response data and applied consistently across all substantive responses using Anthropic Claude 4.6 Sonnet. Responses marked NA, N/A, or providing only brief affirmations (e.g., "Thank you") were excluded as non-substantive. Multiple codes were permitted per response when a single response addressed more than one theme. The denominator for percentage calculations reflects the number of substantive responses in each section. JLB manually reviewed and edited the assignment of comments to responses themes.

### Figure S12. Analysis of open responses using common themes

```{r}
#| fig-width: 9
#| fig-height: 12
#| 
# Load the common theme annotations

survey_comments_data <- read_csv("survey/Comment_Theme_Associations_Full_ed.csv")

# Create a summary table that can be used for making a bar plot

theme_by_section <- survey_comments_data |>
...
# Bar plot of four sections
open_responses <- theme_by_section |>
  filter(count > 0) |>
  mutate(
...
open_responses
# ggsave("survey/open_responses.jpg", width = 9, height = 12)
```
